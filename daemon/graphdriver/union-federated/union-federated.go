...
union-federated driver directory structure

 rootNFSImageLayers - The directory that is used to
 					  mount 'rootLocalImageLayers'
					  directories from remote hosts
  ./
  ├──host1
...
type UnionFederatedOptions struct {
	subdriver string
	nfsroot   string
}
...
var (
	...
	ErrOverlayNotSupported = fmt.Errorf("OverlayFs was not found in /proc/filesystems")

	// AUFS cannot work over BTRFS and AUFS (from graphdriver/aufs.go)
	incompatibleFsMagicAufs = []graphdriver.FsMagic{
		graphdriver.FsMagicBtrfs,
		graphdriver.FsMagicAufs,
	}

	// OverlayFS cannot work over BTRFS, AUFS and ZFS (from graphdriver/overlay.go)
	incompatibleFsMagicOverlay = []graphdriver.FsMagic{
		graphdriver.FsMagicBtrfs,
...
func init() {
	logrus.Debugf("init() 'union-federated'")
	graphdriver.Register("union-federated", Init)
}

type Driver struct {
	root                     string // UnionFederated root
	rootNFSImageLayers       string // NFS root of remote ImageLayers
	rootLocalContainerLayers string // R/W directory for local ContainerLayers
...
func Init(root string, opts []string) (graphdriver.Driver, error) {
	...
	// Check that the requested sub-driver exists and can be used
	switch options.subdriver {
	case "aufs", "overlay":
		err := supportsCheck(options.subdriver)
		if err != nil {
			return nil, err
		}
	default:
		logrus.Debugf("Subdriver is not defined")
		return nil, graphdriver.ErrNotSupported
	}

	err = incompatibleBackingFsCheck(root, options.subdriver)
	if err != nil {
		return nil, err
	}
	...
	if options.nfsroot != "" {
		rootNFSImageLayers = options.nfsroot
	}

	_, err = os.Stat(rootNFSImageLayers)
	if err != nil {
		return nil, fmt.Errorf("Root NFS ImageLayer ",
	...
	// Create the root UnionFederated driver dir and return
...
func incompatibleBackingFsCheck(root string, subdriver string) error {
	...
	incompatibleFsMagic := incompatibleFsMagicAufs
	if subdriver == "overlay" {
		incompatibleFsMagic = incompatibleFsMagicOverlay
	}
...
func parseOptions(opt []string) (UnionFederatedOptions, error) {
	var options UnionFederatedOptions
	options.subdriver = ""
	options.nfsroot = ""
...
func supportsCheck(subdriver string) error {
	// We can try to modprobe overlay first before looking at
	// proc/filesystems for when overlay is supported
	exec.Command("modprobe", subdriver).Run()
	...
	for s.Scan() {
		if strings.Contains(s.Text(), subdriver) {
			if subdriver == "overlay" {
				err := checkOverlayKernelVersion()
				if err != nil {
					logrus.Warnf("You are running linux kernel ",
						"that does not support OverlayFS multy-layer. ",
						"Please upgrade your kernel to 3.19.0 or later")
...
// OverlayFS support Multy-Layer since kernel 3.19
func checkOverlayKernelVersion() error {
	...
	res := kernel.CompareKernelVersion(k,
		&kernel.KernelVersionInfo{Kernel: 3, Major: 19, Minor: 0})
	if res < 0 {
		return graphdriver.ErrNotSupported
	}
...
func (a *Driver) String() string {
	return fmt.Sprintf("uf-%s", a.subdriver)
}
...
// Creates work directories an populates
// metadata file for the given id
...
// Create "id" directory under "mnt" and "diff" directories inside of
// ContainerLayers if isImageLayer == false
// or
// ImageLayers if isImageLayer == true
...
// Umount id directory if it is mounted
...
func (a *Driver) mount(idDesc *IdDesc, mountLabel string) error {
	...
	var (
		...
		work   = path.Join(idDesc.rootPath, "work", idDesc.id)
	)
	...
	case "overlay":
		if err := a.overlayMount(layers, rw, target, work, mountLabel); err != nil {
...
func (a *Driver) unmount(idDesc *IdDesc) error {
	...
	target := path.Join(idDesc.rootPath, "mnt", idDesc.id)
	return Unmount(target, a.subdriver == "aufs")
}
...
// FIXME:
//
// All the bellow code need to review and
// rework according to used sub-driver
//
// FIXME:
...
// NOTE: OverlayFs must support Multi-Layer (linux kernel >=3.19)
func (a *Driver) overlayMount(lowerDirs []string, upperDir, target, workDir, mountLabel string) (err error) {
	defer func() {
		if err != nil {
	...
	b := make([]byte, syscall.Getpagesize()-len(mountLabel)) // room for xino & mountLabel
	bp := copy(b, fmt.Sprintf("upperdir=%s,workdir=%s,lowerdir=%s",
		upperDir, workDir, lowerDirs[0]))

	i := 1
	for ; i < len(lowerDirs); i++ {
		layer := fmt.Sprintf(":%s", lowerDirs[i])
	...
	//logrus.Errorf("Target:[%s], Opts:[%s]",
	//	target, label.FormatMountLabel(string(b[:bp]), mountLabel))

	err = mount("overlay", target, "overlay", 0,
		label.FormatMountLabel(string(b[:bp]), mountLabel))
	if err != nil {
		return
	}
...
func useDirperm() bool {
	enableDirpermLock.Do(func() {
		...
		enableDirperm = true
		if err := Unmount(union, true); err != nil {
...
func Unmount(target string, aufs bool) error {
	if aufs {
